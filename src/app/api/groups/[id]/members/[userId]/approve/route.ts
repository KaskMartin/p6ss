...
export async function POST(
  ...
  try {
    ...
    if (!isGroupOwner && !isGroupAdmin && !isGlobalAdmin) {
      return NextResponse.json({ error: 'Only group admins can approve members' }, { status: 403 })
    }
    ...
    // Approve the user (set need_admin_approve to false)
    await db
      .updateTable('group_members')
      .set({
        need_admin_approve: false,
        updated_at: new Date(),
      })
